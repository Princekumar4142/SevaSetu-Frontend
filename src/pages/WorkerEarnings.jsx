    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-headline-md text-headline-md text-on-surface font-bold">{tr("Earnings & Payouts")}</h1>
            <p className="font-body-md text-body-md text-on-surface-variant">
              {tr("Track gross income, cooperative dividends, welfare reserves, and transfer funds to your bank")}
            </p>
          </div>
        </div>
        <Button
        ...
        >
          ...
          {withdrawing ? tr("Processing...") : `${tr("Withdraw")} ₹${balance.toLocaleString()}`}
        </Button>
      </div>
      ...
      {payoutSuccess && (
        <div className="p-4 bg-emerald-50 text-emerald-800 rounded-xl border border-emerald-200 flex items-center justify-between animate-fade-in">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="font-label-md font-bold">{tr("Instant Payout Initiated!")}</p>
              <p className="font-status-badge text-status-badge text-emerald-700">
                {tr("₹8,450 successfully transferred to your linked HDFC Bank account (•••• 4920) via IMPS.")}
              </p>
            </div>
          </div>
          <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant p-5 shadow-sm">
          ...
          <p className="font-status-badge text-status-badge text-brand-success font-medium mt-1">
            {tr("Ready for instant bank withdrawal")}
          </p>
        </div>
        ...
        <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant p-5 shadow-sm">
          ...
          <p className="font-status-badge text-status-badge text-on-surface-variant mt-1">
            {tr("Quarterly cooperative profit pool")}
          </p>
        </div>
        ...
        <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant p-5 shadow-sm">
          ...
          <p className="font-status-badge text-status-badge text-on-surface-variant mt-1">
            {tr("Life insurance & pension credit")}
          </p>
        </div>
      </div>
      ...
      <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant p-6 shadow-sm">
        <h2 className="font-label-lg text-label-lg font-bold text-on-surface mb-4">
          {tr("Weekly Earnings Activity (Aug 2026)")}
        </h2>
        <div className="grid grid-cols-7 gap-2 pt-6 pb-2 items-end h-48 border-b border-outline-variant">
          {[
            { day: tr("Mon"), amount: 1450, height: "45%" },
            { day: tr("Tue"), amount: 2100, height: "65%" },
            { day: tr("Wed"), amount: 950, height: "30%" },
            { day: tr("Thu"), amount: 3200, height: "90%" },
            { day: tr("Fri"), amount: 1800, height: "55%" },
            { day: tr("Sat"), amount: 3600, height: "100%" },
            { day: tr("Sun"), amount: 2800, height: "80%" },
          ].map((bar) => (
          ...
          {STATEMENTS.map((tx) => (
            <div key={tx.id} className="py-3.5 flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-label-md text-label-md font-bold text-on-surface">{tr(tx.desc)}</p>
